...
def main(config):
    ...
    if svname is None:
        ...
        svname += '_' + config['model'] + '-np2-' + config['model_args']['encoder']
    ...
    f = h5py.File('savefeature_tiered_base.hdf5', 'r')
    data_feature = f['data_feature']
    b_data = np.zeros(shape=(train_dataset.n_classes,512))
    if ep_per_batch != None:
        d_data = np.zeros(shape=(ep_per_batch,train_dataset.n_classes,512))
    for i in range(train_dataset.n_classes):
        b_data[i] = data_feature[i]
    for i in range(ep_per_batch):
            d_data[i] = b_data

    x_data=torch.as_tensor(torch.from_numpy(d_data), dtype=torch.float32)
    x_data = x_data.cuda()
    print(x_data.shape)
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        for data, _ in tqdm(train_loader, desc='train', leave=False):
            ...
            logits,x,logitsdemo = model(x_shot, x_query,x_data)
            logits = logits.view(-1, n_train_way)
            logitsdemo = logitsdemo.view(-1, n_train_way)
            x = x.view(-1, n_train_way)

            loss =F.cross_entropy(x,label)
            acc = utils.compute_acc(x, label)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config',default='configs/train_meta_tiered_demo.yaml')
