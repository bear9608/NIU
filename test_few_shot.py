...
def main(config):
    ...
    f = h5py.File('savefeature_base.hdf5', 'r')
    data_feature = f['data_feature']
    b_data = np.zeros(shape=(64,512))
    if ep_per_batch != None:
        d_data = np.zeros(shape=(ep_per_batch,64,512))
    for i in range(64):
    ...
    if config.get('load') is None:
        model = models.make('meta-baseline-demo', encoder=None)
    else:
        ...
        for data, _ in tqdm(loader, leave=False):
            ...
            with torch.no_grad():
                if not args.sauc:
                    ...
                    logitsdemo = logitsdemo.view(-1, n_way)
                    x = x.view(-1, n_way)
